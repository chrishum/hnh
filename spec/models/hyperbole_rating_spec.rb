...
describe Rating do
  ...
  it "should create a new hyperbole rating given valid attributes" do
    @rating = @statement.hyperbole_ratings.build(@attr)
    @rating.user = @user
    @rating.save!
  end
  ...
  describe "statement associations" do
    ...
    before(:each) do
      @rating = @statement.hyperbole_ratings.create(@attr)
    end
  ...
  describe "validations" do
    ...
    it "should reject duplicate hyperbole ratings" do
      @rating = @statement.hyperbole_ratings.build(@attr)
      @rating.user = @user
      @rating.save!
      
      @dup_rating = @statement.hyperbole_ratings.build(@attr)
